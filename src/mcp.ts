...
const BUILD_GUIDE = [
  ...
  "  - Return shapes: schedule()/every() resolve to { id }. list() resolves to rows { id, tag, title, body, url, icon, image, type, time, cron } (plus `bodies` for a rotation) — enough to rebuild your UI. Cancel one with easyhost.notify.cancel(id).",
